...
func TestSpotFetcherPriceAt(t *testing.T) {
	...
	if !approx(price, 4.52) {
		t.Errorf("expected 4.52, got %f", price)
	}
	...
	if !approx(price, 4.21) {
		t.Errorf("expected 4.21, got %f", price)
	}
}
...
func TestSpotFetcherPriceAtMissing(t *testing.T) {
	...
	if ok {
		t.Error("expected no price when cache is empty")
	}
}
...
func TestSpotFetcherRefetchBackoff(t *testing.T) {
	...
	if calls != 1 {
		t.Errorf("expected 1 call (backoff), got %d", calls)
	}
}
