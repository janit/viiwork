...
func TestLoadDotEnv(t *testing.T) {
	...
	if os.Getenv("ENTSOE_API_KEY") != "test-key-123" {
		t.Errorf("expected test-key-123, got %s", os.Getenv("ENTSOE_API_KEY"))
	}
	if os.Getenv("OTHER_VAR") != "hello" {
		t.Errorf("expected hello, got %s", os.Getenv("OTHER_VAR"))
	}
	if os.Getenv("SPACED") != "value" {
		t.Errorf("expected 'value', got '%s'", os.Getenv("SPACED"))
	}
...
func TestLoadDotEnvNoOverride(t *testing.T) {
	...
	if os.Getenv("EXISTING_VAR") != "original" {
		t.Errorf("expected original, got %s", os.Getenv("EXISTING_VAR"))
	}
}
