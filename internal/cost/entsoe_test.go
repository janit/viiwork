...
func TestParsePrices(t *testing.T) {
	...
	if !approx(prices[0].CentsKWh, 4.52) {
		t.Errorf("price[0]: expected 4.52, got %f", prices[0].CentsKWh)
	}
	if !approx(prices[1].CentsKWh, 4.21) {
		t.Errorf("price[1]: expected 4.21, got %f", prices[1].CentsKWh)
	}
	if !approx(prices[2].CentsKWh, -0.15) {
		t.Errorf("price[2]: expected -0.15, got %f", prices[2].CentsKWh)
	}
}
