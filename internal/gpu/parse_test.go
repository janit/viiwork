...
func TestParseROCmSMI(t *testing.T) {
	...
	if s0 == nil {
		t.Fatal("missing gpu 0")
	}
	if s0.Utilization != 85.0 {
		t.Errorf("gpu0 util: expected 85.0, got %f", s0.Utilization)
	}
	if int(s0.VRAMTotalMB) != 16368 {
		t.Errorf("gpu0 vram total: expected 16368, got %d", int(s0.VRAMTotalMB))
	}
	if int(s0.VRAMUsedMB) != 14199 {
		t.Errorf("gpu0 vram used: expected ~14199, got %d", int(s0.VRAMUsedMB))
	}
	s1 := findByGPU(samples, 1)
	if s1 == nil {
		t.Fatal("missing gpu 1")
	}
	if s1.Utilization != 42.0 {
		t.Errorf("gpu1 util: expected 42.0, got %f", s1.Utilization)
	}
}
...
func TestParseROCmSMIEmpty(t *testing.T) {
	...
	if len(samples) != 0 {
		t.Errorf("expected 0, got %d", len(samples))
	}
}
...
func TestParseROCmSMIPower(t *testing.T) {
	...
	if got := findByGPU(samples, 0).PowerW; got != 163.0 {
		t.Errorf("gpu0 power: expected 163.0, got %f", got)
	}
	// An idle card still draws real power; that separation is what per-model
	// energy attribution divides by.
	if got := findByGPU(samples, 1).PowerW; got != 21.0 {
		t.Errorf("gpu1 power: expected 21.0, got %f", got)
	}
}
...
func TestParseROCmSMIWithoutPower(t *testing.T) {
	...
	if got := findByGPU(samples, 0).PowerW; got != 0 {
		t.Errorf("expected no power, got %f", got)
	}
	if got := findByGPU(samples, 0).Utilization; got != 85 {
		t.Errorf("utilisation must survive a missing power field, got %f", got)
	}
}
...
func TestParseROCmSMIAlternatePowerKey(t *testing.T) {
	...
	if len(samples) != 1 || samples[0].PowerW != 142.0 {
		t.Errorf("expected 142.0 W from the alternate key, got %+v", samples)
	}
}
