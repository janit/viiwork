...
Applies to **viiwork 2.0 and later**. viiwork 2 changed the shape of every
payload in section 4 — one node per machine, one port, and a membership list
where version 1 had a node plus a list of polled peers. If you are reading a
`local` / `peers` structure, you are talking to a 1.x node and want that
version's document.
...
viiwork is an LLM inference mesh. **One node runs per machine**, supervising
every model configured there, and serves its API and dashboards on **port
8086**. Nodes find each other by themselves — over a tailnet, by mDNS on a LAN,
or from a seed address — so there is no peer list to maintain anywhere.

Any node is a complete entry point: **ask one node and it reports the whole
mesh.** You do not need to reach every machine, and you do not need to know the
topology in advance. One HTTP call to one reachable node gives you the fleet;
one held-open stream gives you live activity for all of it.

Requests follow free slots rather than round-robin: a node runs a request
locally if it has a free slot, otherwise forwards to the member with the most
free slots, otherwise queues it. A caller does not choose a host, and should
not try to.

Most of what the API exposes is in-memory state that a restart clears —
activity, prompt history, metrics. Two things are durable and survive a
restart: the **alias table** in the node's state directory, and the **energy
store** on hosts that enable it.

## 2. Access and trust

**Inference and read endpoints are unauthenticated.** No API keys, no tokens,
no sessions. Anything that can open a TCP connection to port 8086 can list
models, read prompt history and send a completion. This is deliberate — viiwork
expects a private network, where reachability *is* the authorization model —
but it has consequences you must design around:

- **Whatever you build must not become a path from an untrusted network to a
  node.** If your application is reachable more widely than viiwork is, your
  application's own authentication is the only thing in between. Put the
...
Three things are *not* open, and they are the exceptions to remember:

- **Mesh membership** is either *secured* — a shared 32-byte secret, encrypted
  gossip and signed forwards — or explicitly *open*. `mesh` on `/v1/cluster`
  tells you which. This protects the mesh from joining, not the API from being
  read.
- **Alias writes** (`PUT`/`DELETE` on `/v1/aliases/<name>`) need a signature
  from that secret in a secured mesh, and must come from the node's own machine
  in an open one. Reading the alias table is open like everything else.
- **Chassis power control** acts only on hosts in the node's configured
  allowlist, published as `power_control.hosts`. There is no wildcard.
...
PATH B — DIRECT FROM THE BROWSER (FALLBACK)
  ...
  Viewer's browser must reach the node itself · needs CORS · no auth gate
```
...
Needs the node's CORS allowlist to name your origin. Take it only when Path A
genuinely
...
expose `X-Gpu-Backend`, `X-Viiwork-Node`, `X-Viiwork-Origin`,
`X-Viiwork-Queued-Ms`, `X-Viiwork-Alias` and `X-Viiwork-Model`.
...
Whole-mesh snapshot: every member the node knows, each with the status it last
reported. This is the one endpoint a fleet overview needs.

```json
{
  "view": "node-a",
  "mesh": "secured",
  "models": ["some-model-27B", "another-model-8b"],
  "members": [
    {
      "node": "node-a",
      "addr": "198.51.100.10",
      ...
      "status": {
        "node": "node-a",
        "node_id": "viiwork-4f133f9145e6a4eb",
        "ver": "v2.0.0",
        "addr": "198.51.100.10",
        "api_port": 8086,
        "uptime_s": 3312,
        "host_mem_total_mb": 64198,
        "host_mem_used_mb": 16848,
        "prompt_history": 1000,
        "energy_kwh_24h": 15.9,
        "energy_kwh_30d": 169.0,
        "power":  { "watts": 978, "available": true, "source": "dcmi" },
        ...
        "models": [ {
          ...
          "backends": [ {
            ...
            "gpus": [0, 1], "pid": 15, "rss_mb": 10547,
            "slots": 2, "busy": 1, "respawns": 0, "uptime_s": 3298
          } ]
        } ]
      }
    }
  ],
  "power_control": { "hosts": ["node-a", "node-b"] }
}
```

| Field | Meaning |
|---|---|
| `view` | The node that answered. Everything else is its view of the mesh. |
| `mesh` | `secured` or `open`. See §2 — it describes who may *join*, not who may read. |
| `models` | Every model served anywhere in the mesh, deduplicated. The quick answer to "what can I ask for". |
| `members[].node` | The member's name, which is its hostname unless configured otherwise. **This is the grouping key for anything measured per machine** — it is a real name, not derived from the address it was dialled on. |
| `members[].state` | `alive` · `dead` · `left`. A node that stops gracefully goes `left`; one that vanishes goes `dead` after the mesh's suspicion window. `meshapi` also defines `suspect` for other implementations; viiwork never reports it. Keep showing a dead member rather than hiding it. |
| `members[].role` | `node` (serves models) or `gateway` (routes only, no models, null status). |
...
| `status.host_mem_*` | Host RAM, `used` being `MemTotal - MemAvailable`, so page cache is not counted as pressure. Coarsened on the pushed stream — see §6. |
| `power_control.hosts` | Machines this node may power-control. Rows for the fleet should be the **union** of members and this list: a powered-off machine is absent from the mesh entirely, and this is the only place its name still exists. |

Everything per-machine is reported once, because there is one node per machine.
Version 1 needed a deduplication rule here; version 2 does not.

### `GET /v1/capacity`

What the router polls, once a second, to decide where a request goes. Cheaper
than `/v1/cluster` and the honest answer to "is there room right now".

```json
{
  "node": "node-a", "ver": "v2.0.0",
  "models": [ { "name": "some-model-27B", "engine": "llamacpp",
                "slots": 2, "busy": 0, "queued": 0, "ctx": 49152,
                "backends": 1, "healthy_backends": 1 } ]
}
```

Free capacity is `slots - busy`, and `healthy_backends` of `0` means the model
...
real id carries a quant and a version.

```json
{ "aliases": [ {
  "name": "stable-coder", "target": "some-model-27B", "fallbacks": [],
  "resolved": "some-model-27B", "state": "ok",
  "updated_at": "2026-09-12T15:10:26.992Z", "updated_by": "node-a"
} ] }
```

| Field | Meaning |
|---|---|
| `state` | `ok` — resolves to its target · `fallback` — the target cannot serve, a fallback is answering · `unavailable` — nothing can serve it, requests get 503 · `shadowed` — a real model now has this name, which wins. |
| `resolved` | What a request for `name` will actually run on, or `null` when nothing can. |
| `fallbacks` | Tried in order when the target cannot serve. |

Reading is open; writing needs a signature (§2). An alias may be pre-staged
against a model no one serves yet — it reads `unavailable` until that model
appears, then starts resolving on its own.

### `GET /v1/status`

The asked node only. It is exactly the `status` object shown inside each member
...
absent from `members` entirely: that is what a powered-off machine looks like,
and it is why the list is published at all.

### `GET /v1/models`

OpenAI-shaped model list, aggregated across the whole mesh, with `owned_by`
saying where each name comes from:
...
`local` is served by the node you asked, `peer` by another member, `alias` is a
name from the alias table, and `pipeline` a configured multi-step chain. A
...
Container-style health for the node you ask, shaped for a Docker `healthcheck`:

```json
{"status": "ok", "version": "v2.0.0", "node": "node-a",
 "uptime_seconds": 3312, "backends_healthy": 6, "backends_total": 6}
```

**200** when at least one backend is healthy, or when the machine has no models
configured at all — a pure router is a valid node. **503** with
`"status": "unhealthy"` when models are configured and not one backend can
serve, which includes the whole cold-load window after a restart and a node
still joining the mesh.

It answers for one machine, so use it as a reachability and readiness signal,
not as fleet health: `/v1/cluster` is what tells you the mesh is well.
...
| `node_id`, `hostname`, `addr` | Which member the event came from. `addr` is empty for the node you are connected to and set for other members; you need it for prompt lookups. |
...
Standard OpenAI chat completions, streaming or not, routed to whichever member
has a free slot for the requested model. `/v1/completions` and `/v1/embeddings`
share the same routing, headers and errors. Relevant only if your integration
includes a prompt box.

The `model` may be a real model id, an alias, or a pipeline name — `/v1/models`
lists all three and the caller does not need to know which it has.

Response headers say what actually happened:

| Header | Meaning |
|---|---|
| `X-Viiwork-Node` | The member that ran it. Always present. |
| `X-Viiwork-Origin` | The member that *received* it, present only when the request was forwarded. Absent means the node you asked ran it itself. |
| `X-Gpu-Backend` | The backend id that executed it, `<model>/<n>`. |
| `X-Viiwork-Queued-Ms` | Milliseconds spent in the origin queue, present only when it waited. |
| `X-Viiwork-Alias`, `X-Viiwork-Model` | Present when an alias was resolved: the name asked for, and the real model that answered. |
...
against members the node already knows. A host that is not serving the model answers
...
- **Capacity** — the last **`activity.prompt_history` requests per node**,
  ...
  own value as `prompt_history` on `/v1/status`, and so in each member's
  `status` on `/v1/cluster`. Read an absent value as "unknown", never as zero.
  **Size any client-side list from the largest value you see rather than
  hardcoding one**, or you will evict rows the owning node could still answer a
  lookup for.
...
|---|---|---|
| Other members' jobs / activity | live | merged event streams |
| Local backend + GPU state | ~5 s | health-check tick |
| Members' slots, busy and queued | ~1 s | `mesh.capacity_poll` |
| Members' full status, GPU load | ~5 s | member status poll |
...
**Check what the node is actually running.** Every member's build is in
`status.ver` on `/v1/cluster`. A 1.x node answers a completely different shape
— `local` and `peers` instead of `members` — so if those keys are present you
are on the wrong document, not looking at a bug.

**Expect a mixed fleet during a rolling upgrade.** Machines are converted one
at a time, so several versions of the contract are live at once. Wire fields
are additive and omitted when a node cannot say, so **handle a field's absence
rather than its emptiness**, and never read a missing number as zero.
