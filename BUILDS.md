...
- The two `Dockerfile`s and the two compose files are intentionally side by side at the repo root, not hidden in subdirectories. Both tracks are first-class citizens.
- `docker-compose.yaml` is the active per-node compose file (gitignored, generated by `setup-node.sh`).
- `docker-compose.yaml.example` is the stable-track example committed to the repo.
- `docker-compose.gfx906.yaml` is the experimental-track example. Older than the stable-track example because the experimental build came later.
- `docker-compose.soak-{prod,fork}.yaml` are the dedicated A/B soak compose files used by `bench-harness/run_overnight_soak.sh`. Not for normal use.
