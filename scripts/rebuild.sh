...
# Volumes are left alone: a node's state volume (its alias table) must never
# be deleted by a routine rebuild, and a stopped node's volume counts as unused.
...
echo "==> WARNING: /health did not answer within 3 minutes. Check: docker compose logs"
exit 1
